...
import (
	...
	planq "github.com/planq-network/planq/types"
)
...
func GetTransferSenderRecipient(packet channeltypes.Packet) (
	...
	// and change the bech32 human readable prefix (HRP) of the sender to `plq`
	sender, err = planq.GetPlanqAddressFromBech32(data.Sender)
	if err != nil {
		return nil, nil, "", "", sdkerrors.Wrap(err, "invalid sender")
	}

	// validate the recipient bech32 address from the counterparty chain
	// and change the bech32 human readable prefix (HRP) of the recipient to `plq`
	recipient, err = planq.GetPlanqAddressFromBech32(data.Receiver)
	if err != nil {
