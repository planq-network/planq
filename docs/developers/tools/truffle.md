...
Learn how to deploy a simple Solidity-based smart contract to Planq using the Truffle environment {synopsis}
...
This will allow your contract to connect to your Planq local node.
...
You should see incoming deployment logs in the Planq daemon Terminal tab for each transaction (one to deploy `Migrations.sol` and the other to deploy `Counter.sol`).
...
Now, you can run the Truffle tests using the Planq node using the `test` command:
