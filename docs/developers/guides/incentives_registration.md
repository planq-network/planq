...
Learn how to register smart contract incentives through [Planq Governance](../../users/governance/overview.md). {synopsis}
...
2. Promote understanding of the `x/incentives` Module.
    ...
    Make sure to mention the [`x/incentives` Module Documentation](../../../x/incentives/spec/README.md) along with a brief explanation of what the `x/incentives` Module does. It's also a good idea to link the [Planq Token Model](https://medium.com/evmos/the-evmos-token-model-edc07014978b), since incentives are directly intertwined with Planq tokenomics!
...
Remember to provide links to the relevant [Commonwealth Planq community](https://commonwealth.im/evmos) discussions concerning your proposal, as well as the [proposal on testnet](#submit-the-proposal-to-testnet).
...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`planqd`](../../validators/quickstart/binary.md), use the following command with `register-incentive`:
...
- `$ALLOCATION`: Denominations and percentage of the total rewards (25% of block distribution) to be allocated to users that interact and spend gas using the `$CONTRACTADDRESS` (eg. `"0.005000000000000000aplanq"` will distribute 0.5% of out of the 25% tokens minted on each daily epoch rewards).
...
```bash
planqd tx gov submit-proposal register-incentive 0xFCd2Ce20ef8ed3D43Ab4f8C2dA13bbF1C6d9512F 0.050000000000000000aplanq 13 --description=$DESCRIPTION --title=$PROPOSALTITLE
```
...
```bash
planqd tx gov submit-proposal register-incentive --proposal=<path_to_json>
```
...
Planq supports the following metrics related to the `x/incentives` module, which can be collected for incentive analysis:
...
- [Planq Supported Telemetry Metrics](https://docs.planq.network/protocol/telemetry.html)
