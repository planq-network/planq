...
Learn how to register interoperable ERC-20s through [Planq Governance](../../users/governance/overview.md). {synopsis}
...
To [submit the proposal](../../users/governance/submitting.md) to testnet through the command line with [`planqd`](../../validators/quickstart/binary.md), use the following command with `register-erc20`:

```bash
planqd tx gov submit-proposal register-erc20 ERC20_ADDRESS...\
  --title=<title> \
...
```bash
planqd tx gov submit-proposal register-erc20 --proposal=<path/to/proposal.json>
```
