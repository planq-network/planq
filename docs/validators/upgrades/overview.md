...
Planned upgrades are coordinated scheduled upgrades that use the [upgrade module](https://docs.planq.network/modules/upgrade/) logic. This facilitates smoothly upgrading Planq to a new (breaking) software version as it automatically handles the state migration for the new release.

### Governance Proposal

Governance Proposals are a mechanism for coordinating an upgrade at a given height or time using an [`SoftwareProposal`](https://docs.planq.network/modules/upgrade/01_concepts.html#proposal).
...
Unplanned upgrades are upgrades where all the validators need to gracefully halt and shut down their nodes at exactly the same point in the process. This can be done by setting the `--halt-height` flag when running the `planqd start` command.
...
Upgrades can be categorized as breaking or non-breaking according to the Semantic versioning ([Semver](https://semver.org/)) of the corresponding software [release version](https://github.com/planq-network/planq/releases) (*i.e* `vX.Y.Z`):
...
To upgrade the genesis file, you can either fetch it from a trusted source or export it locally using the `planqd export` command.
