...
`planqd` is the all-in-one command-line interface. It supports wallet management, queries and transaction operations {synopsis}
...
After you have obtained the latest `planqd` binary, run:
...
::: tip
You can also enable auto-completion with the `planqd completion` command. For example, at the start of a bash session, run `. <(planqd completion)`, and all `planqd` subcommands will be auto-completed.
:::

### Config and data directory

By default, your config and data are stored in the folder located at the `~/.planqd` directory.
...
To specify the `planqd` config and data storage directory; you can update it using the global flag `--home <directory>`

### Configuring the Node

The Cosmos SDK automatically generates two configuration files inside `~/.planqd/config`:
...
To set pruning, adjust the `pruning` parameter in the `~/.planqd/config/app.toml` file.
...
If you would like to change your nodes pruning strategy then you must do so when the node is initialized. Passing a flag when starting `plq` will always override settings in the `app.toml` file, if you would like to change your node to the `everything` mode then you can pass the `---pruning everything` flag when you call `planqd start`.
...
We can view the default client config setting by using `planqd config` command:
...
Alternatively, we can directly make the changes to the config values in one place at client.toml. It is under the path of `.planqd/config/client.toml` in the folder where we installed evmos:
...
A list of commonly used `planqd` commands. You can obtain the full list by using the `planqd -h` command.
