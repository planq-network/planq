...
speed and transaction amount. For Planq, we are talking about close to
...
I believe this was disabled by default on Planq, but listing it in any
...
will risk database corruption if the `planqd` is killed for any reason.
...
Below is the disk usage after two weeks of Planq Arsia Mons testnet. The default
