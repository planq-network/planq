...
Thank you for considering making contributions to Planq and related repositories!

Planq uses [Tendermint’s coding repo](https://github.com/tendermint/coding) for overall information on repository
...
1. Either [open](https://github.com/planq-network/planq/issues/new/choose)
   or [find](https://github.com/planq-network/planq/issues) an issue you have identified and would like to contribute to
   resolving.
2. Participate in thoughtful discussion on that issue.
3. If you would like to contribute:
    1. If the issue is a proposal, ensure that the proposal has been accepted by the Planq team.
    ...
    4. Follow standard Github best practices:
        1. Fork the repo
        2. Branch from the HEAD of `development`(For core developers working within the Planq repo, to ensure a
...
some [good first issues](https://github.com/planq-network/planq/issues?q=label%3A%22good+first+issue%22).

## <span id="adr">Architecture Decision Records (ADR)</span>

When proposing an architecture decision for Planq, please create
...
at `https://github.com/planq-network/planq`, the code should never exist
at `$GOPATH/src/github.com/planq-network/planq`. Instead, we use `git remote` to add the fork as a new remote for the
original repo,`$GOPATH/src/github.com/planq-network/planq`, and do all the work there.

For instance, to create a fork and work on a branch of it, you would:

1. Create the fork on github, using the fork button.
2. Go to the original repo checked out locally. (i.e. `$GOPATH/src/github.com/planq-network/planq`)
3. `git remote rename origin upstream`
4. `git remote add origin git@github.com:ivanshukhov/planq.git`

Now `origin` refers to my fork and `upstream` refers to the  version. So I can `git push -u origin master` to
update my fork, and make pull requests to  from there. Of course, replace `ivanshukhov` with your git handle.
...
with [gogoproto](https://github.com/gogo/protobuf) to generate code for use in Planq.
...
4. Create your feature branch from `development` either on `github.com/planq-network/planq`, or your fork (
   using `git remote add origin`).
5. Before submitting a pull request, begin `git rebase` on top of `development`.

## <span id="testing">Testing</span>

Planq uses [GitHub Actions](https://github.com/features/actions) for automated testing.

## <span id="updating_doc">Updating Documentation</span>

If you open a PR on the Planq repo, it is mandatory to update the relevant documentation in `/docs`. Please refer to
...
Planq utilizes [semantic versioning](https://semver.org/).
...
**Note**: Planq's team currently cuts releases on a need to have basis. We will announce a more
