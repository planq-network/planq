...
# Set moniker and chain-id for planq (Moniker can be anything, chain-id must be an integer)
planqd init "$MONIKER" --chain-id "$CHAINID"

# Change parameter token denominations to aplanq
jq '.app_state.staking.params.bond_denom="aplanq"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.crisis.constant_fee.denom="aplanq"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.gov.deposit_params.min_deposit[0].denom="aplanq"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.evm.params.evm_denom="aplanq"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
jq '.app_state.inflation.params.mint_denom="aplanq"' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
node_address=$(planqd keys list | grep  "address: " | cut -c12-)
...
# 0xA61808Fe40fEb8B3433778BBC2ecECCAA47c8c47 || planq15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz
jq -r --arg amount_to_claim "$amount_to_claim" '.app_state.bank.balances += [{"address":"planq15cvq3ljql6utxseh0zau9m8ve2j8erz89m5wkz","coins":[{"denom":"aplanq", "amount":$amount_to_claim}]}]' "$GENESIS" > "$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
...
## 1. Back to `planqd keys add` step, init more keys
## 2. Back to `planqd add-genesis-account` step, add balance for those
## 3. Clone this ~/.planqd home directory into some others, let's say `~/.clonedplanqd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedplanqd/config/gentx/` folders into the original `~/.planqd/config/gentx`
...
planqd start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aplanq --json-rpc.api eth,txpool,personal,net,debug,web3
