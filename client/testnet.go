...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		accStakingTokens := sdk.TokensFromConsensusPower(5000, planqtypes.PowerReduction)
		coins := sdk.Coins{
			sdk.NewCoin(planqtypes.AttoPhoton, accStakingTokens),
		}

		genBalances = append(genBalances, banktypes.Balance{Address: addr.String(), Coins: coins.Sort()})
		genAccounts = append(genAccounts, &planqtypes.EthAccount{
			BaseAccount: authtypes.NewBaseAccount(addr, nil, 0, 0),
			CodeHash:    common.BytesToHash(evmtypes.EmptyCodeHash).Hex(),
		})

		valTokens := sdk.TokensFromConsensusPower(100, planqtypes.PowerReduction)
		createValMsg, err := stakingtypes.NewMsgCreateValidator(
			sdk.ValAddress(addr),
			valPubKeys[i],
			sdk.NewCoin(planqtypes.AttoPhoton, valTokens),
		...
		customAppTemplate, customAppConfig := config.AppConfig(planqtypes.AttoPhoton)
	...
	if err := initGenFiles(clientCtx, mbm, args.chainID, planqtypes.AttoPhoton, genAccounts, genBalances, genFiles, args.numValidators); err != nil {
		return err
	}
